...
class Todo(db.Model):
    ...
    due_date = db.Column(db.Date, default=datetime.now, onupdate=datetime.now)
...
def add():
    todo=Todo(task=request.form['todoitem'],due_date=dparser.parse(request.form['due_date'], fuzzy=True).date(),complete=False)
    db.session.add(todo)
    db.session.commit()
...
def edit(action,id=None):
    ...
    if action in ('complete','move'):
        ...
        todo.due_date = dparser.parse(request.form['due-date'], fuzzy=True).date()
